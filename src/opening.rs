// The opening book.
//
// The first moves of a game are not worth searching. They have been played by hand
// hundreds of thousands of times over, and a book is the record of what came of it:
// the engine looks the position up and plays what is written down, until the game
// leaves what anyone has written down and the search takes over.
//
// A position usually has several moves written down for it, each with the weight of
// how much play is behind it. The move is drawn at random against those weights, so
// the main line comes up most of the time and the sidelines now and then - always
// playing the heaviest move would mean playing the same game every time.
//
// The draw is the position mixed with a seed the book was opened with. That makes it
// the same move for the same position all game long, so analysing a position twice
// does not change what the engine says about it, while a new game with a new book
// plays something else.
//
// The books in assets are Polyglot books, the format practically every engine reads.
// A book is a flat array of 16 byte entries sorted by position key: eight bytes of
// key, two of move, two of weight, four of learning data that nothing here uses, all
// of it big endian. Sorted is what makes the lookup a binary search over the file
// itself, with nothing read in up front and nothing parsed.
//
// There are three of them, asked one after another. No book covers everything, and
// where the first one runs out the second often carries on for another few moves -
// the game only leaves the book once none of the three has anything to say.
//
// The catch is the key. Polyglot hashes with a zobrist scheme of its own, so the
// engine's own hash is no use here and the key has to be built a second way, out of
// the 781 numbers at the bottom of this file. The two schemes differ in more than
// their numbers: polyglot mixes in an en passant square only when a pawn of the side
// to move could actually take it, and it marks the side to move by mixing a key in
// for white where the engine mixes one in for black.
//
// A move out of the book is a move somebody wrote into a file, so none of it is taken
// on trust: the squares are looked up in the legal moves of the position, and a book
// that does not belong to the game being played simply finds nothing.

use crate::board::Board;
use crate::board::castling::{CASTLE_FLAGS, CastleSide, king_castle_square};
use crate::board::chess_move::Move;
use crate::board::piece::{Color, Piece, PieceType};
use crate::board::square::{file_of, offset};
use std::ops::Range;
...
// eight bytes of key, two of move, two of weight, four of learning data
const ENTRY_SIZE: usize = 16;

// where the keys that are about something other than a piece on a square begin
const CASTLING_KEYS: usize = 768;
const EN_PASSANT_KEYS: usize = 772;
const WHITE_TO_MOVE_KEY: usize = 780;

// one of the books: the file as it lies, read where it is needed rather than parsed
// into anything
#[derive(Clone, Copy)]
struct Book {
    entries: &'static [u8],
...
impl Book {
    fn new(entries: &'static [u8]) -> Book {
        Book { entries }
    }

    // the entries filed under one key: a book is sorted by key, so they are a run of
    // it, and the binary search only has to find where that run starts
    fn entries_for(&self, key: u64) -> Range<usize> {
        ...
        let mut end = low;
        while end < count && self.key_at(end) == key {
            end += 1;
        }
    ...
    fn key_at(&self, index: usize) -> u64 {
        u64::from_be_bytes(self.bytes(index, 0))
    }

    // the move and what the book thinks of it, out of the middle four bytes
    fn move_at(&self, index: usize) -> (u16, u16) {
        (
            u16::from_be_bytes(self.bytes(index, 8)),
            u16::from_be_bytes(self.bytes(index, 10)),
        )
    }

    // N bytes of one entry, starting `offset` bytes into it
    fn bytes<const N: usize>(&self, index: usize, offset: usize) -> [u8; N] {
        let start = index * ENTRY_SIZE + offset;
...
pub struct OpeningBook {
    // asked in order, each one only because the book before it had nothing to say
    books: [Book; 3],
    // what the draw is mixed with: one seed per book, so one opening line per game
    seed: u64,
}

impl OpeningBook {
    // a book of its own for every game, drawn against a seed nothing else will have
    pub fn new() -> OpeningBook {
        OpeningBook::seeded(seed_from_the_clock())
    }

    // the same book every time, for a caller that wants the same game every time
    pub fn seeded(seed: u64) -> OpeningBook {
        OpeningBook::over(
            [Book::new(BOOK_1), Book::new(BOOK_2), Book::new(BOOK_3)],
            seed,
        )
    }

    fn over(books: [Book; 3], seed: u64) -> OpeningBook {
    ...
    pub fn move_for(&self, board: &Board) -> Option<Move> {
        ...
        // the same number every time this position is asked about, since a position
        // analysed twice should not change its mind
        let draw = mix(key ^ self.seed);

        // generated at most once, however many of the books have to be asked
        let mut legal = None;

        for &book in &self.books {
            let entries = book.entries_for(key);
            if entries.is_empty() {
                continue;
            }

            let legal = legal.get_or_insert_with(|| board.legal_moves());

            if let Some(chess_move) = draw_from(book, entries, board, legal, draw) {
...
fn draw_from(
    ...
    for index in entries {
        let (encoded, weight) = book.move_at(index);

        if let Some(chess_move) = decode(board, legal, encoded) {
            total += u32::from(weight);
            moves.push((u32::from(weight), chess_move));
        }
    }

    let (_, first) = *moves.first()?;

    // nothing written down here weighs anything, so there is no line to draw from
    if total == 0 {
        return Some(first);
    }

    // the weights laid end to end, and the one point of that line the draw lands on
    let mut point = (draw % u64::from(total)) as u32;
    for (weight, chess_move) in moves {
        if point < weight {
            return Some(chess_move);
        }
        point -= weight;
    ...
    // the point lies inside the line, so the loop always finds it
    Some(first)
}

// splitmix64 on the way out: two positions next to each other in the book differ in
// a bit or two, and this is what turns that into two unrelated draws
...
// a number that is not the same one the last game started with
...
fn decode(board: &Board, moves: &[Move], encoded: u16) -> Option<Move> {
    ...
    let promotion = match (encoded >> 12) & 0b111 {
    ...
    let to = castle_target(board, from, to).unwrap_or(to);

    moves.iter().copied().find(|chess_move| {
        chess_move.from == from && chess_move.to == to && chess_move.promotion == promotion
    })
}

// polyglot writes a castle as the king taking its own rook, e1h1, where the generator
// writes the square the king really lands on, e1g1
fn castle_target(board: &Board, from: u8, to: u8) -> Option<u8> {
    let king = board.piece_at(from)?;
    let rook = board.piece_at(to)?;

    if !king.is(PieceType::King) || !rook.is(PieceType::Rook) || king.color() != rook.color() {
        return None;
    }

    let side = match file_of(to) {
        7 => CastleSide::King,
        _ => CastleSide::Queen,
    };

    Some(king_castle_square(king.color(), side))
}

// the position under the key the book files it by
fn polyglot_key(board: &Board) -> u64 {
    let mut key = 0;

    for square in 0..64 {
        if let Some(piece) = board.piece_at(square) {
            key ^= RANDOM_KEYS[64 * piece_index(piece) + square as usize];
        }
    }

    for (index, (color, side)) in CASTLE_FLAGS.into_iter().enumerate() {
        if board.castling_rights().get(color, side) {
            key ^= RANDOM_KEYS[CASTLING_KEYS + index];
        }
    }

    if let Some(target) = capturable_en_passant(board) {
        key ^= RANDOM_KEYS[EN_PASSANT_KEYS + file_of(target) as usize];
    }

    // the other way round from the engine's own hash, which mixes in black to move
    if board.turn() == Color::White {
        key ^= RANDOM_KEYS[WHITE_TO_MOVE_KEY];
    }

    key
}

// black pawn 0, white pawn 1, black knight 2, and so on up to white king 11 - the
// order the piece keys are laid out in
...
// the en passant square as polyglot counts it: only where a pawn of the side to move
// stands ready to take it, whether or not taking it would be any good
fn capturable_en_passant(board: &Board) -> Option<u8> {
...
// The 781 numbers polyglot hashes with: 64 squares for each of the twelve pieces,
// then the four castling sides, the eight en passant files, and white to move.
#[rustfmt::skip]
static RANDOM_KEYS: [u64; 781] = [
...
mod tests {
    ...
    // the moves the book writes down for the starting position, heaviest first
    const OPENINGS: [(u8, u8); 8] = [
        (12, 28), // e2e4
        (11, 27), // d2d4
        (6, 21),  // g1f3
        (10, 26), // c2c4
    ...
    // a book that draws the same way every run, so a test can say which move it draws
    fn book(seed: u64) -> OpeningBook {
    ...
    fn an_en_passant_nobody_can_take_is_left_out_of_the_key() {
        ...
        assert!(
            ...
            "e3 is the en passant square"
        );
        assert_eq!(polyglot_key(&board), 0x823c_9b50_fd11_4196);
    }

    // and with a pawn standing next to it, the same square counts
    #[test]
    fn an_en_passant_that_can_be_taken_is_counted() {
        let mut board = start_position();
        board.make_move_from_squares(12, 28, None); // e2e4
        board.make_move_from_squares(51, 35, None); // d7d5
        board.make_move_from_squares(28, 36, None); // e4e5
        board.make_move_from_squares(53, 37, None); // f7f5, which the e5 pawn can take

        assert_eq!(polyglot_key(&board), 0x22a4_8b5a_8e47_ff78);
    }

    // the king stepping out gives up both white castles, which the key has to show
    #[test]
    fn castling_rights_given_up_change_the_key() {
        let mut board = start_position();
        board.make_move_from_squares(12, 28, None); // e2e4
        board.make_move_from_squares(51, 35, None); // d7d5
        board.make_move_from_squares(28, 36, None); // e4e5
        board.make_move_from_squares(53, 37, None); // f7f5
        board.make_move_from_squares(4, 12, None); // Ke1e2

        assert_eq!(polyglot_key(&board), 0x652a_607c_a3f2_42c1);
    }

    // whatever the draw, what comes out is one of the moves written down here
    #[test]
    fn the_book_opens_with_a_move_it_has_written_down() {
        let board = start_position();

        for seed in 0..50 {
            let opening = book(seed).move_for(&board).expect("the book knows the start");

            assert!(
                OPENINGS.contains(&(opening.from, opening.to)),
                "the book opened with {}, which it does not list",
                opening.coordinates()
            );
        }
    }

    // the draw itself, pinned down: these two seeds land in the first two moves of the
    // line, and nothing about how the weights are laid out may quietly change that
    #[test]
    fn the_draw_lands_where_the_weights_put_it() {
        let board = start_position();

        let first = book(0).move_for(&board).expect("the book knows the start");
        let second = book(1).move_for(&board).expect("the book knows the start");

        assert_eq!((first.from, first.to), (12, 28), "seed 0 did not draw e2e4");
        assert_eq!((second.from, second.to), (11, 27), "seed 1 did not draw d2d4");
    }

    // the point of the weights: e2e4 and d2d4 carry about 42% of the book each, and
    // the four sidelines below g1f3 carry well under 1% between them
    #[test]
    fn the_weights_decide_how_often_a_move_comes_up() {
        let board = start_position();
        ...
        for seed in 0..1000 {
            let opening = book(seed).move_for(&board).expect("the book knows the start");
            let played = (opening.from, opening.to);

            if played == OPENINGS[0] || played == OPENINGS[1] {
                main_lines += 1;
            }
            if OPENINGS[4..].contains(&played) {
                sidelines += 1;
            }
        }

        assert!(
            (600..950).contains(&main_lines),
            "e2e4 and d2d4 came up {main_lines} times in 1000, which is not the 84% they weigh"
        );
        assert!(
            sidelines < 50,
            "the sidelines came up {sidelines} times in 1000, far more than they weigh"
        );
    }

    // the same position twice is the same move: analysing a position again must not
    // change what the engine says about it
    #[test]
    fn one_book_answers_a_position_the_same_way_every_time() {
        let board = start_position();
        let book = book(7);

        let asked_once = book.move_for(&board);
        let asked_again = book.move_for(&board);

        assert_eq!(asked_once, asked_again);
    }

    // and it answers, with a move that is legal in the position it answered
    #[test]
    fn the_book_answers_the_first_move() {
        let mut board = start_position();
        board.make_move_from_squares(12, 28, None); // e2e4

        let answer = book(0).move_for(&board).expect("the book knows 1. e4");

        assert!(
            board.legal_moves().contains(&answer),
            "the book answered with {}, which is not legal here",
            answer.coordinates()
        );
    }

    // 1. a4 is not in the first book, nor in the second, and the third has five
    // answers to it - so a move only comes back if all three are really asked
    #[test]
    fn a_position_the_first_books_lack_is_looked_for_in_the_next() {
        let mut board = start_position();
        board.make_move_from_squares(8, 24, None); // a2a4

        let key = polyglot_key(&board);
        assert!(
            Book::new(BOOK_1).entries_for(key).is_empty(),
            "the first book holds 1. a4 after all, so this tests nothing"
        );
        assert!(
            Book::new(BOOK_2).entries_for(key).is_empty(),
            "the second book holds 1. a4 after all, so this tests nothing"
        );

        let answer = book(0)
            .move_for(&board)
            .expect("the third book answers 1. a4");
    ...
    // the order itself: two books with nothing in them are walked past rather than
    // taken for the end of the line
    #[test]
    fn an_empty_book_is_not_the_end_of_the_line() {
        let board = start_position();
        let book = OpeningBook::over(
            [Book::new(&[]), Book::new(&[]), Book::new(BOOK_1)],
            0,
        );

        let opening = book.move_for(&board).expect("the third book knows the start");

        assert_eq!(
            (opening.from, opening.to),
            (12, 28),
            "the third book drew {} instead of e2e4",
            opening.coordinates()
        );
    }

    // a position none of the three ever wrote down is where the book stops and the
    // search takes over
    #[test]
    fn a_position_the_book_never_saw_is_not_in_it() {
        let mut board = Board::new();
        ...
        assert_eq!(book(0).move_for(&board), None);
    }

    // the entries of one key are a run of the file, and the search has to find where
    // that run starts - the starting position is the one run easy to check by hand
    #[test]
    fn the_entries_of_a_key_are_found_as_a_run() {
        let book = Book::new(BOOK_1);
        let entries = book.entries_for(0x463b_9618_1691_fc9c);

        assert!(!entries.is_empty(), "the start position is not in the book");
        for index in entries {
            assert_eq!(book.key_at(index), 0x463b_9618_1691_fc9c);
        }
    }
}
